...
colors = pl.cm.Blues(np.linspace(0,1,24))
...
for tg_idx,tg in enumerate(input_tg):
                    ...
                    for res_idx,res in enumerate(input_res):
                          ...
                          #       ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),'--',color=colors[idx_line_plot],label=easys+'_'+atype+'_w'+res,linewidth=1.5)
                           ...
                           if res == '10' and (atype == 'FC1' or atype == 'FC2' or atype == 'FC3') :
                              ax.plot(time4plot,line4plot,':',color=colors[idx_line_plot],label=easys+'_'+atype+'_w'+res,linewidth=3)
                              #ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),':',color=colors[idx_line_plot],label=easys+'_'+atype+'_w'+res,linewidth=1.5)
                           elif (res == '08' or (res == '08_12' and easys ==  'EAS5' ) ) and (atype == 'FC1' or atype == 'FC2' or atype == 'FC3') :
                                 ax.plot(time4plot,line4plot,'--',color=colors[idx_line_plot],label=easys+'_'+atype+'_w'+res,linewidth=3)
                                 #ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),'--',color=colors[idx_line_plot],label=easys+'_'+atype+'_w'+res,linewidth=1.5)
                           else:
                              try:
    ...
    # OBS
    ax.plot(time4plot,var_obs,'o-',color='black',label='OBS',linewidth=1.5)
    print ('Obs max:',np.max(var_obs))
    if flag_oldtpxo != 1:
       ax.plot(time4plot,np.squeeze(tpxo_sig)+obs_mean,'--',color='black',label='TPXO',linewidth=1.5)
    else:
       print ('old tpxo')
       ax.plot(time4plot,np.squeeze(tpxo_sig)-tpxo_diffs+obs_mean,'--',color='black',label='TPXO',linewidth=1.5)
    ...
    box = ax.get_position()
    leg = plt.legend(loc='lower right', ncol=2,  shadow=True, fancybox=True, fontsize=15)
    ax.grid('on')
